 ...
 *
 * Wiced Transport in RAW UART mode uses a single circular buffer to recieve the data
 * Application should specify the size of this recieve buffer when doing transport init
 */
...
/**
 *  \brief Function to drive UART TX Drive mode
 *
 *  \param [in] enable 0 - tristate tx / 1 - enable tx pin
 *  \return none
 */
void wiced_hci_change_tx_drive_mode(wiced_bool_t enable);

/**
 *  \brief Function to drive UART RTS mode
 *
 *  \param [in] enable enable 0 - tristate rts / 1 - enable rts pin
 *  \return none
 */
void wiced_hci_change_rts_drive_mode(wiced_bool_t enable);
